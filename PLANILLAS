diff --git a/herramienta.py b/herramienta.py
new file mode 100644
index 0000000000000000000000000000000000000000..9038f45b402b3c602da78ad4e5cd55e8f7ccc433
--- /dev/null
+++ b/herramienta.py
@@ -0,0 +1,343 @@
+import csv
...
+            match = next((row for row in self.external_data if row.get(self.external_key, "") == key_value), None)
+            if match:
...
+
+    def set_external_configuration(self, *, key_field: str, fields: list[str], data: list[dict[str, str]]) -> None:
...
+            messagebox.showwarning("Registro vacío", "Debe diligenciar al menos un campo antes de guardar.")
+            return
...
+        ttk.OptionMenu(key_frame, self.key_field, self.key_field.get(), *FIELDS).pack(side="left", padx=5)
+
+        load_button = ttk.Button(self, text="Cargar base externa (CSV)", command=self.load_external_file)
...
+        self.field_listbox = tk.Listbox(list_frame, selectmode="multiple", exportselection=False, height=10)
+        self.field_listbox.pack(fill="both", expand=True, pady=5)
+
+        save_button = ttk.Button(self, text="Guardar configuración externa", command=self.save_configuration)
...
+            messagebox.showerror("Error", f"No se pudo leer el archivo seleccionado.\n{error}")
+            return
+
+        if not self.external_data:
+            messagebox.showwarning("Sin datos", "El archivo seleccionado no contiene registros.")
+            return
+
...
+
+        messagebox.showinfo("Base cargada", "La base externa se cargó correctamente. Seleccione los campos a cruzar.")
+
+    def save_configuration(self) -> None:
+        if not self.external_data:
+            messagebox.showwarning("Sin base externa", "Debe cargar primero una base externa.")
+            return
+
+        selected_indices = self.field_listbox.curselection()
+        if not selected_indices:
+            messagebox.showwarning("Sin campos", "Seleccione al menos un campo externo.")
+            return
+
...
+            )
+
+        self.data_manager.set_external_configuration(key_field=key_field, fields=selected_fields, data=self.external_data)
+        messagebox.showinfo("Configuración guardada", "Se guardó la configuración para el cruce de datos.")
+
+
...
+            summary_values = [record.get(self.data_manager.display_fields[0], f"Registro {index + 1}")]
+            self.records_tree.insert("", tk.END, iid=str(index), values=summary_values)
+
+    def save_fields(self) -> None:
+        selected_indices = self.field_listbox.curselection()
+        if not selected_indices:
+            messagebox.showwarning("Sin selección", "Seleccione al menos un campo para la visualización.")
+            return
+        selected_fields = [self.field_listbox.get(i) for i in selected_indices]
+        self.data_manager.set_display_fields(selected_fields)
+        messagebox.showinfo("Campos guardados", "Se actualizaron los campos para la visualización.")
+        self.populate_records()
+
...
+        except IndexError:
+            messagebox.showerror("Error", "No se pudo recuperar el registro seleccionado.")
+            return
+
...
+            ttk.Label(form.inner, text=field, style="FieldLabel.TLabel").grid(row=index, column=0, sticky="w", padx=5, pady=4)
+            value_widget = ttk.Label(form.inner, text=value or "—", wraplength=400, justify="left")
